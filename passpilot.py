...
class AdvancedPasswordAnalyzer:
    ...
    def get_strength_feedback(self, entropy):
        ...
        if entropy < 28:
            return "Critical", "#c0392b", 0
        elif entropy < 36:
            return "Very Weak", "#e74c3c", 1
        elif entropy < 50:
            return "Weak", "#e67e22", 2
        elif entropy < 65:
            return "Fair", "#f39c12", 3
        elif entropy < 80:
            return "Good", "#f1c40f", 4
        elif entropy < 100:
            return "Strong", "#2ecc71", 5
        elif entropy < 120:
            return "Very Strong", "#27ae60", 6
        else:
            return "Exceptional", "#16a085", 7
    ...
    def get_suggestions(self, password):
        ...
        return suggestions if suggestions else ["• Your password is excellent!"]
...
class PassPilotSupreme(tk.Tk):
    ...
    def create_menu(self):
        ...
        view_menu.add_checkbutton(label="Show Passwords", variable=self.show_password, command=self.toggle_password_visibility)
    ...
    def create_widgets(self):
        ...
        ttk.Label(header, text="Military-Grade Password Security Toolkit", 
    ...
    def create_generator_tab(self):
        ...
        self.notebook.add(gen_tab, text="🎲 Generator")
        ...
        # Presets
        preset_frame = ttk.LabelFrame(container, text="Quick Presets", padding=15)
        ...
        # Custom settings
        settings_frame = ttk.LabelFrame(container, text="Custom Settings", padding=15)
        settings_frame.pack(fill="x", pady=(0, 15))
        
        # Length slider
        length_frame = ttk.Frame(settings_frame)
        length_frame.pack(fill="x", pady=5)
        ttk.Label(length_frame, text="Length:", style="Subtitle.TLabel").pack(side="left", padx=(0, 10))
        ...
        ttk.Checkbutton(chars_frame, text="Lowercase (a-z)", variable=self.lower_var).grid(row=0, column=0, sticky="w", pady=3)
        ttk.Checkbutton(chars_frame, text="Uppercase (A-Z)", variable=self.upper_var).grid(row=0, column=1, sticky="w", pady=3, padx=20)
        ttk.Checkbutton(chars_frame, text="Numbers (0-9)", variable=self.digits_var).grid(row=1, column=0, sticky="w", pady=3)
        ttk.Checkbutton(chars_frame, text="Symbols (!@#$%)", variable=self.symbols_var).grid(row=1, column=1, sticky="w", pady=3, padx=20)
        ttk.Checkbutton(chars_frame, text="Exclude ambiguous (0OIl1|)", variable=self.exclude_ambiguous).grid(row=2, column=0, columnspan=2, sticky="w", pady=3)
        
        # Generated password display
        output_frame = ttk.LabelFrame(container, text="Generated Password", padding=15)
        ...
        ttk.Button(btn_frame, text="🎲 Generate Password (Ctrl+G)", 
                  command=self.generate_password).pack(fill="x", pady=2)
        ttk.Button(btn_frame, text="📝 Generate Passphrase (Ctrl+P)", 
                  command=self.generate_passphrase).pack(fill="x", pady=2)
        ttk.Button(btn_frame, text="📋 Copy to Clipboard (Ctrl+C)", 
    ...
    def create_analyzer_tab(self):
        ...
        self.notebook.add(ana_tab, text="🔍 Analyzer")
        
        container = ttk.Frame(ana_tab)
        container.pack(fill="both", expand=True, padx=20, pady=20)
        
        # Input
        input_frame = ttk.LabelFrame(container, text="Password Analysis", padding=15)
        input_frame.pack(fill="x", pady=(0, 15))
        
        ttk.Label(input_frame, text="Enter password to analyze:").pack(anchor="w", pady=(0, 5))
        ...
        viz_frame = ttk.LabelFrame(container, text="Strength Visualization", padding=15)
        ...
        self.feedback_label = ttk.Label(viz_frame, text="Enter a password to analyze", 
                                       font=("Segoe UI", 12, "bold"), anchor="center")
        self.feedback_label.pack(fill="x")
        
        # Detailed metrics
        metrics_frame = ttk.LabelFrame(container, text="Detailed Metrics", padding=15)
        metrics_frame.pack(fill="x", pady=(0, 15))
        
        self.entropy_label = ttk.Label(metrics_frame, text="Entropy: - bits", anchor="w")
        self.entropy_label.pack(fill="x", pady=2)
        
        self.crack_time_label = ttk.Label(metrics_frame, text="Est. crack time: -", anchor="w")
        self.crack_time_label.pack(fill="x", pady=2)
        
        self.char_entropy_label = ttk.Label(metrics_frame, text="Character diversity: -", anchor="w")
        self.char_entropy_label.pack(fill="x", pady=2)
        
        # Suggestions
        suggest_frame = ttk.LabelFrame(container, text="Improvement Suggestions", padding=15)
        ...
        ttk.Button(breach_frame, text="🌐 Check if Exposed in Data Breach", 
    ...
    def create_history_tab(self):
        ...
        self.notebook.add(hist_tab, text="📜 History")
        
        container = ttk.Frame(hist_tab)
        container.pack(fill="both", expand=True, padx=20, pady=20)
        
        ttk.Label(container, text="Session Password History (In-Memory Only)", 
                 style="Subtitle.TLabel").pack(pady=(0, 10))
        
        ttk.Label(container, text="⚠️ Passwords are stored only for this session and are never written to disk.",
    ...
    def generate_password(self):
        ...
        self.status_bar.config(text=f"Password generated ({len(password)} characters)")
    ...
    def generate_passphrase(self):
        ...
        self.status_bar.config(text=f"Passphrase generated")
        self.refresh_history()
    ...
    def copy_to_clipboard(self):
        ...
        self.status_bar.config(text="Copied! Clipboard will clear in 30 seconds...")
    ...
    def clear_clipboard(self):
        ...
        self.clipboard_clear()
        self.status_bar.config(text="Clipboard cleared for security")
    ...
    def analyze_password(self, *args):
        ...
        if not password:
            ...
            self.feedback_label.config(text="Enter a password to analyze", foreground="#95a5a6")
            self.entropy_label.config(text="Entropy: - bits")
            self.crack_time_label.config(text="Est. crack time: -")
            self.char_entropy_label.config(text="Character diversity: -")
        ...
        self.feedback_label.config(text=f"{strength_text} Password", foreground=color)
        
        # Update metrics
        self.entropy_label.config(text=f"Entropy: {entropy} bits (pool-based)")
        self.crack_time_label.config(text=f"Est. crack time: {crack_time}")
        self.char_entropy_label.config(text=f"Character diversity: {char_entropy} bits (Shannon)")
    ...
    def check_pwned_threaded(self):
        ...
        if not password:
            messagebox.showinfo("Info", "Please enter a password to check.")
            return
        
        self.pwned_label.config(text="🔄 Checking...", foreground="#3498db")
        self.status_bar.config(text="Checking breach database...")
    ...
    def update_pwned_result(self, count, error):
        ...
        if error:
            self.pwned_label.config(text=f"❌ {error}", foreground="#e74c3c")
            self.status_bar.config(text="Breach check failed")
        elif count > 0:
            msg = f"⚠️ EXPOSED! Found {count:,} times in breaches"
            self.pwned_label.config(text=msg, foreground="#e74c3c")
            self.status_bar.config(text="Password is compromised!")
        else:
            msg = "✅ Safe! Not found in known breaches"
            self.pwned_label.config(text=msg, foreground="#2ecc71")
            self.status_bar.config(text="Password passed breach check")
    ...
    def copy_from_history(self):
        ...
        if not selection:
            messagebox.showinfo("Info", "Please select a password from history.")
            return
